...
/** Scala Native factory for [[HostBackedPreopen]] — backs a wasi
  * preopen with a real on-disk directory through `java.nio.file`.
  *
  * Native's javalib supplies the same NIO surface as the JVM (channels,
  * `Files`, `Paths`, attribute views), so this file is identical to
  * the JVM impl — duplicating rather than introducing a JVM+Native
  * shared source folder keeps the build layout simple.
  *
  * Usage:
  *
  * {{{
  ...
  * }}}
  *
  * The JS build supplies a Node `fs`-backed object with the same
  * `HostPreopen.fromDir` signature, so user code compiles unchanged
