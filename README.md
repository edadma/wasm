...
```bash
sbt 'interpJVM/Test/run'    # 627 interpreter tests
...
Total: **849 tests** on JVM (627 interp + 208 wasi + 14 cli) — all three backends green for `interp` and `wasi`. Three of those are end-to-end integration tests against real rustc-built `wasm32-wasip1` binaries.
