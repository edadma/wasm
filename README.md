...
```bash
sbt 'interpJVM/Test/run'    # 620 interpreter tests
...
Total: **842 tests** on JVM (620 interp + 208 wasi + 14 cli) — all three backends green for `interp` and `wasi`. Three of those are end-to-end integration tests against real rustc-built `wasm32-wasip1` binaries.
